// ============================================================
//  LUCK WORLD SERVER v5.0
//  3 PARALLEL WORKERS | RANDOM API | INFINITE RETRY
//  50 API + 50 RPC = 100 ENDPOINTS PER COIN
//  Display only when ALL 12 balances complete
//  While waiting → next mnemonic already generating
// ============================================================
...
// ============================================================
//  FETCH WITH TIMEOUT (3 seconds for speed)
// ============================================================
async function fetchT(url, opts = {}, ms = 3000) {
...
// ============================================================
//  BASE ENDPOINTS (25-30 unique per coin)
// ============================================================
const BASE_RPC = {
    btc: [
    ...
    eth: [
        {t:'evm_rpc',u:'https://rpc.ankr.com/eth'},{t:'evm_rpc',u:'https://ethereum.publicnode.com'},
        {t:'evm_rpc',u:'https://cloudflare-eth.com'},{t:'evm_rpc',u:'https://eth.llamarpc.com'},
        {t:'evm_rpc',u:'https://eth-mainnet.public.blastapi.io'},{t:'evm_rpc',u:'https://1rpc.io/eth'},
        {t:'evm_rpc',u:'https://ethereum.blockpi.network/v1/rpc/public'},{t:'evm_rpc',u:'https://gateway.tenderly.co/public/ethereum'},
        {t:'evm_rpc',u:'https://rpc.mevblocker.io'},{t:'evm_rpc',u:'https://eth.drpc.org'},
        {t:'evm_rpc',u:'https://rpc.flashbots.net'},{t:'evm_rpc',u:'https://mainnet.eth.cloud.ava.do'},
        {t:'evm_rpc',u:'https://eth.gateway.pm'},{t:'evm_rpc',u:'https://rpc.ethereal.engineering'},
        {t:'evm_rpc',u:'https://eth.archivelabs.org'},{t:'evm_rpc',u:'https://eth.rencode.io'},
        {t:'evm_rpc',u:'https://api.mycryptoapi.com/eth'},{t:'evm_rpc',u:'https://nodes.mewapi.io/rpc/eth'},
        {t:'evm_rpc',u:'https://rpc.scaffoldeth.io'},{t:'evm_rpc',u:'https://eth-mainnet.rpc.blxrbdn.com'},
        {t:'evm_rpc',u:'https://eth-mainnet.nodereal.io/v1/35deee6b1b2e4fb0b2ec74f9'},
        {t:'evm_rpc',u:'https://ethereumnodelight.app.runonflux.io'},
        {t:'evm_rpc',u:'https://eth-mainnet.gateway.pokt.network/v1/5f345397d19a2a60073471b7'},
        {t:'evm_rpc',u:'https://rpc.ankr.com/eth'},{t:'evm_rpc',u:'https://ethereum.publicnode.com'}
    ],
    bnb: [
        {t:'evm_rpc',u:'https://bsc-dataseed.binance.org'},{t:'evm_rpc',u:'https://bsc-dataseed1.defibit.io'},
        {t:'evm_rpc',u:'https://bsc-dataseed2.ninicoin.io'},{t:'evm_rpc',u:'https://bsc-dataseed3.defibit.io'},
        {t:'evm_rpc',u:'https://bsc-dataseed4.defibit.io'},{t:'evm_rpc',u:'https://bsc-dataseed1.binance.org'},
        {t:'evm_rpc',u:'https://bsc-dataseed2.binance.org'},{t:'evm_rpc',u:'https://bsc-dataseed3.binance.org'},
        {t:'evm_rpc',u:'https://bsc-dataseed4.binance.org'},{t:'evm_rpc',u:'https://rpc.ankr.com/bsc'},
        {t:'evm_rpc',u:'https://bsc.publicnode.com'},{t:'evm_rpc',u:'https://bsc-mainnet.public.blastapi.io'},
        {t:'evm_rpc',u:'https://bsc-rpc.publicnode.com'},{t:'evm_rpc',u:'https://binance.llamarpc.com'},
        {t:'evm_rpc',u:'https://bsc-mainnet.nodereal.io/v1/d7b1239279e34217b3d3645e'},
        {t:'evm_rpc',u:'https://bsc.getblock.io/mainnet/'},{t:'evm_rpc',u:'https://bsc.nodereal.io/v1/d7b1239279e34217b3d3645e'},
        {t:'evm_rpc',u:'https://bsc-dataseed.binance.org'},{t:'evm_rpc',u:'https://bsc-dataseed1.defibit.io'},
        {t:'evm_rpc',u:'https://bsc-dataseed2.ninicoin.io'},{t:'evm_rpc',u:'https://bsc-dataseed3.defibit.io'},
        {t:'evm_rpc',u:'https://bsc-dataseed4.defibit.io'},{t:'evm_rpc',u:'https://rpc.ankr.com/bsc'},
        {t:'evm_rpc',u:'https://bsc.publicnode.com'},{t:'evm_rpc',u:'https://binance.llamarpc.com'}
    ],
    trx: [
...
// ============================================================
//  EXPAND TO 100 PER COIN (50 API + 50 RPC = 100)
// ============================================================
const RPC = {};
for (const c of ALL) {
    RPC[c] = [];
    while (RPC[c].length < 100) RPC[c].push(...BASE_RPC[c]);
    RPC[c] = RPC[c].slice(0, 100);
}

// ============================================================
//  RANDOM API PICKER (never pick same index twice)
// ============================================================
function randomRPC(coin, excludeSet) {
    const eps = RPC[coin];
    const total = eps.length;
    // Collect available indices
    const avail = [];
    for (let i = 0; i < total; i++) {
        if (!excludeSet.has(i)) avail.push(i);
    }
    if (avail.length === 0) return null;
    // Random pick from available
    return { idx: avail[Math.floor(Math.random() * avail.length)], ep: null };
    // Note: we return idx, caller gets ep from RPC[coin][idx]
}
...
// ============================================================
//  SINGLE API CALL (one endpoint, one address)
// ============================================================
async function callAPI(coin, ep, addr) {
    ...
    if (coin === 'dash') return await callDASH(ep, addr);
    return await callUTXO(ep, coin, addr);
}
...
// --- UTXO (LTC) ---
async function callUTXO(ep, coin, a) {
...
// ============================================================
//  BALANCE CHECK WITH RANDOM RETRY
//  Fire 2 random APIs → if both fail → 2 more different → repeat
//  Until success OR all 100 APIs exhausted
// ============================================================
async function checkBalanceWithRetry(coin, addr) {
    ...
    while (tried.size < total) {
        // Pick 2 random untried indices
        const avail = [];
        for (let i = 0; i < total; i++) {
            if (!tried.has(i)) avail.push(i);
        }
        // Shuffle available and pick up to 2
        ...
        // Fire ALL picked APIs simultaneously (parallel, not in line!)
        const results = await Promise.allSettled(
            batch.map(i => {
                const ep = RPC[coin][i];
                return callAPI(coin, ep, addr).catch(() => ({...ERR}));
            })
        );
        ...
        // All failed in this batch → loop and try 2 more different APIs
    }
    // All 100 APIs exhausted
    return {...ERR};
}
...
// ============================================================
//  PROCESS ONE WALLET (derive → fire 12 checks → wait ALL → display)
// ============================================================
async function processWallet() {
    ...
    // Fire ALL 12 balance checks SIMULTANEOUSLY (6 coins × 2 types)
    // Each check runs its own retry loop with random APIs
    const promises = {};
    ...
    // Wait for ALL 12 to complete (no display until ALL done)
    await Promise.allSettled(
        ALL.flatMap(c => [promises[c].comp, promises[c].uncomp])
    );

    // Get results (all promises now resolved)
    const results = {};
    for (const coin of ALL) {
        results[coin] = {
    ...
    // Emit ONE complete event with ALL data
    io.emit('wallet', {
        ...
        apiCallCount,
        totalAddresses: state.checkCount * 12,
        bgCheckCount: state.checkCount * 12
    });
}

// ============================================================
//  3 PARALLEL WORKERS
//  While worker 1 waits for APIs, workers 2 & 3 generate & check
// ============================================================
async function worker(id) {
    ...
    while (true) {
        try {
            ...
            console.error('[WORKER ' + id + '] ERR:', e.message);
...
// ============================================================
//  SPEED COUNTER (setInterval - always accurate)
// ============================================================
...
io.on('connection', function(socket) {
    ...
    socket.emit('init', {
        ...
        foundData: state.foundData.slice(-50)
    });
    socket.on('coins_toggle', function(d) {
        if (d && d.coins && Array.isArray(d.coins)) {
            state.selCoins = d.coins.filter(function(c){ return ALL.includes(c); });
    ...
    socket.on('disconnect', function() { console.log('[DISC] ' + socket.id); });
});

// ============================================================
//  START SERVER + 3 WORKERS
// ============================================================
app.use(express.static('public'));
server.listen(PORT, function() {
    console.log('============================================');
    console.log('  LUCK WORLD v5.0');
    console.log('  3 Parallel Workers | Random API');
    console.log('  Infinite Retry | 100 APIs/coin');
    console.log('  Port: ' + PORT);
    console.log('============================================');
    for (const c of ALL) {
        console.log('  ' + CS[c].padEnd(5) + ': ' + RPC[c].length + ' endpoints');
    }
    console.log('============================================');

    io.emit('log', { msg: '<span style="color:#60a5fa;font-weight:900;font-size:13px">LUCK WORLD v5.0 - 3 WORKERS - RANDOM API</span>' });
    io.emit('log', { msg: '<span style="color:#22c55e">Infinite retry: jab tak success na aye APIs try karta hai</span>' });
    io.emit('log', { msg: '<span style="color:#22d3ee">3 Workers parallel: waiting me dusra mnemonic generate hota hai</span>' });
    io.emit('log', { msg: '<span style="color:#f97316">100 APIs per coin (50 API + 50 RPC) | Random pick</span>' });
    io.emit('log', { msg: '' });
